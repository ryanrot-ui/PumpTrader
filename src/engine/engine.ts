...
import { rpcEndpoints } from "@/lib/env";
import {
  collectMetrics,
  forgetToken,
  getPoolLiquidityUsd,
...
/**
 * The trading engine. Two execution models share this class:
 *
 *   Long-running worker (`npm run engine`, src/engine/index.ts):
 *     start() wires interval timers and a WebSocket scanner and runs until
 *     SIGTERM — for a VPS or a local machine.
 *
 *   Bounded serverless cycles (netlify/functions/*):
 *     runFor(ms) runs the full engine (5s position monitoring, polling
 *     scanner with a persisted cursor) for a bounded window inside a
 *     Background Function, then exits with status "idle"; a per-minute
 *     scheduled function relaunches it when the DB lease (EngineState
 *     heartbeat) goes stale. runOnce(budgetMs) is the degraded fallback
 *     that fits inside a regular scheduled function: one
 *     monitor/scan/evaluate pass per minute.
 *
 *   Either way ALL durable state lives in Postgres (watchlist, positions,
 *   settings, control queue, scanner cursor), so bounded cycles lose
 *   nothing — this is the same crash-recovery path the worker always had.
 *
 * Lifecycle:
...
export interface EngineOptions {
  /**
   * Poll-only migration scanning (no WebSocket subscription). Set for
   * bounded serverless cycles, where a WS subscription would be torn down
   * every few minutes; the poller + persisted cursor catch every migration.
   */
  pollOnlyScanner?: boolean;
}
...
export class TradingEngine {
  ...
  private scanner: MigrationScanner | null = null;
  ...
  /**
   * Build the migration scanner. Reads the persisted poll cursor so bounded
   * cycles resume scanning exactly where the previous cycle stopped, and
   * persists every cursor advance back to the EngineState row.
   */
  private async initScanner(): Promise<MigrationScanner> {
    const state = await prisma.engineState
      .findUnique({ where: { id: ENGINE_STATE_ID }, select: { scannerCursor: true } })
      .catch(() => null);
    this.scanner = new MigrationScanner(
      ...
      {
        pollOnly: this.opts.pollOnlyScanner,
        initialCursor: state?.scannerCursor ?? null,
        onCursor: (signature) =>
          updateEngineState({ scannerCursor: signature }).catch(() => {}),
      }
    );
    return this.scanner;
  }
  ...
  /**
   * Crash/cycle recovery — rebuild all in-memory state from Postgres:
   * open-position count (informational), the watchlist, and a persisted
   * emergency stop. Called on every start, including each bounded cycle.
   */
  private async recover(): Promise<void> {
    const [openPositions, state, recent] = await Promise.all([
      prisma.position.count({ where: { status: "OPEN" } }),
      prisma.engineState.findUnique({ where: { id: ENGINE_STATE_ID } }),
    ...
    // An emergency stop outlives the process that received it.
    this.emergencyStopped = state?.status === "emergency_stopped";
    ...
    logger.info(
      "engine",
      `recovered ${openPositions} open position(s), ${recent.length} watched token(s)` +
        (this.emergencyStopped ? " — EMERGENCY STOP still in effect" : "")
    );
  }
  ...
  async start(): Promise<void> {
    ...
    await (await this.initScanner()).start();
    ...
    // If an emergency stop was in effect when the previous process ended,
    // finish the job: exit anything still open (no-op when nothing is).
    if (this.emergencyStopped) void this.emergencyExitAll();
    logger.info("engine", "engine running — scanning for Pump.fun migrations");
  }

  /**
   * @param finalStatus what to leave in EngineState: "stopped" for a real
   * shutdown (long-running worker), "idle" when a bounded serverless cycle
   * ends and the scheduler will start the next one. An in-effect emergency
   * stop always wins so it survives cycle boundaries.
   */
  async stop(finalStatus: "stopped" | "idle" = "stopped"): Promise<void> {
    ...
    await updateEngineState({
      status: this.emergencyStopped ? "emergency_stopped" : finalStatus,
    }).catch(() => {});
    logger.info("engine", finalStatus === "idle" ? "engine cycle complete" : "engine stopped");
  }

  /**
   * Bounded full-fidelity run for a serverless Background Function: the
   * complete engine (5s monitoring, polling scanner, hot-reloading settings)
   * for `ms`, then a clean stop with status "idle" so the scheduler
   * relaunches the next cycle immediately.
   */
  ...
  /**
   * One bounded pass for a regular scheduled function (degraded fallback
   * when Background Functions are unavailable): exits are evaluated first,
   * then the scan/evaluate work uses whatever budget remains.
   */
  async runOnce(budgetMs = 8_000): Promise<void> {
    const deadline = Date.now() + budgetMs;
    await this.config.reload();
    await this.recover();
    await this.stateTick();
    if (this.emergencyStopped) {
      await this.emergencyExitAll(); // idempotent: no open positions → no-op
    } else {
      await this.monitorPositions(); // exits first — highest priority
      const scanner = await this.initScanner();
      if (Date.now() < deadline - 3_000) {
        await scanner.pollOnce().catch((e) =>
          logger.warn("scanner", `poll failed: ${(e as Error).message}`)
        );
      }
      if (Date.now() < deadline - 1_000) {
        await this.evaluateWatchlist(deadline - 500);
      }
    }
    await this.publishHealth();
    await updateEngineState({
      status: this.emergencyStopped ? "emergency_stopped" : "idle",
    }).catch(() => {});
  }
  ...
  private async evaluateWatchlist(deadline?: number): Promise<void> {
    ...
    for (let i = 0; i < tokens.length; i += BATCH) {
      if (deadline && Date.now() > deadline) return; // bounded cycle out of budget
